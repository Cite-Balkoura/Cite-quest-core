...
public class QuestDetailsGUI extends FastInv {
    ...
    public QuestDetailsGUI(Manager manager, Player p, Quest quest) {
        ...
        setItem(0, arrow_back, inventoryClickEvent -> { new QuestGUI(manager, p); });
        ...
        final int current_step_of_player = Manager.playerQuests.get(p.getUniqueId()).get(quest.getIdentifier());
        final boolean isQuestOver = manager.getQuestManager().isQuestOver(p, quest.getIdentifier());
        ...
        for (QuestSteps step : quest.getSteps()) {
            if (current_step_of_player >= slot_steps - 26){ // if step has been discovered
                ...
                setItem(slot_steps, stepItemstack.toItemStack(), inventoryClickEvent -> {
